...
public class HtmlUtile {
	...
	public String getHtmltableFormate(LinkedHashMap<String, Integer> mapping,
			String locationName) {
	...
	//
	public String getHtmlTableWithMultipleCol(
			ArrayList<PatientScheduled> mapping) {
		...
		buf.append("<html>"
				...
				+ "<table style='font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif; border-spacing: 0px;  border-style:none; width: 100%;'>"
				+ "<caption style='padding: 8px; padding-top: 12px; padding-bottom: 12px;font-size: 150%;'><code>");
		buf.append(StringUtils.isBlank(missedFupConditions(mapping.get(0))) ? mapping
				.get(0).getFacilityScheduled() : mapping.get(0)
				.getFacilityName());
		buf.append("</code></caption>"
				+ "<tr>"
	...
	public String getHtmlTableWithSet(Set<String> locationName) {
		...
		while (iterator.hasNext()) {
			...
			buf.append(
					...
					.append(index++)
					.append("</td><td style = 'border: 1px solid #ddd;padding: 8px;'>")
					.append(setElement).append("</td></tr>");
		}
		buf.append("</table>" + "</body>" + "</html>");
		String html = buf.toString();
