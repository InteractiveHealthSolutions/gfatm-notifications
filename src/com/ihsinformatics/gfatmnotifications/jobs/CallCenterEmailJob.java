	...
	protected void scheduledPatient() {
		...
		if (!scheduledPatientList.isEmpty()) {
			...
			while (iterator.hasNext()) { 
				...
				if (supervisorEmail == null) {
						...
						if (patientScheduled.getMvfReturnVisitDate() != null){
							    ...
							    removeLocation = openMrsUtil.getPatientByScheduledFacilityName(patientScheduled
										.getFacilityName());
							}else{
						...
						if (patientScheduled.getMvfReturnVisitDate() != null){
							patientScheduledResult = openMrsUtil.getPatientByScheduledFacilityName(patientScheduled
									.getFacilityName());
						}else{
							if (patientScheduled.getFacilityScheduled() != null ) {
					      patientScheduledResult = openMrsUtil.getPatientByScheduledFacilityName(patientScheduled
			...
			if (!locationsSet.isEmpty() && locationsSet !=null) {
			    ...
			    sendEmail(watcherEmail,HtmlUtile.getInstance().getMessageFormate(ccConcernPerson,bodyMessage,"Call Center"),noUpdateAvailableSubject);
		}
	}
