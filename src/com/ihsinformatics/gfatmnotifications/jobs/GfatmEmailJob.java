...
public class GfatmEmailJob implements Iemail {
	...
	private void petDailyEmailReport(DateTime dateFrom) {
		...
		if (!factPet.isEmpty()) {
			for (PetFact petTable : factPet) {
				Email emailVal = openMrsUtil.getEmailByLocationId(petTable
						.getLocationId());
				if (emailVal == null) {
			...
			sendEmail(businesAnalystEmail, HtmlUtile
					.getInstance().getMessageFormate(gfatmConcernPerson.toString(),bodyMessage.toString(),"PET"),
					 subjectpet);
	...
	private void childhoodDailyEmailReport(DateTime dateFrom2) {
		...
		if (!factChildhood.isEmpty()) {
			...
			sendEmail(businesAnalystEmail, HtmlUtile
					.getInstance().getMessageFormate(gfatmConcernPerson,bodyMessage.toString(),"Childhood"),
					subjectchildhood);
			log.warning("No updates are avaiable...");
		}
	}
	...
	private void fastDailyEmailReport(DateTime dateFrom2) {
		...
		if (!factFast.isEmpty()) {
			for (FastFact factTable : factFast) {
				Email emailVal = openMrsUtil.getEmailByLocationId(factTable
						.getLocationId());
				if (emailVal == null) {
					log.warning("This Location:"
			...
			//businesAnalystEmail
			sendEmail(businesAnalystEmail, HtmlUtile
					.getInstance().getMessageFormate(gfatmConcernPerson,bodyMessage.toString(),"Fast"),
					 subjectfast);
			log.warning("No updates are avaiable...");
		}
