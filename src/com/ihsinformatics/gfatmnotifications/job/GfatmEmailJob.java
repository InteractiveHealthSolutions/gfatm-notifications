...
import com.ihsinformatics.gfatmnotifications.service.EmailService;
...
public class GfatmEmailJob implements EmailService {
	...
	private void petDailyEmailReport(DateTime dateFrom) {
		...
		if (!factPet.isEmpty()) {
			...
			sendEmail(businesAnalystEmail, HtmlUtile.getInstance().getMessageFormate(gfatmConcernPerson.toString(),
	...
	private void childhoodDailyEmailReport(DateTime dateFrom2) {
		...
		if (!factChildhood.isEmpty()) {
			...
			sendEmail(businesAnalystEmail,
					HtmlUtile.getInstance().getMessageFormate(gfatmConcernPerson, bodyMessage.toString(), "Childhood"),
					subjectchildhood);
			log.warning("No updates are avaiable...");
		}
	}
	...
	private void fastDailyEmailReport(DateTime dateFrom2) {
		...
		if (!factFast.isEmpty()) {
			...
			// businesAnalystEmail
			sendEmail(businesAnalystEmail,
					HtmlUtile.getInstance().getMessageFormate(gfatmConcernPerson, bodyMessage.toString(), "Fast"),
					subjectfast);
			log.warning("No updates are avaiable...");
		}
