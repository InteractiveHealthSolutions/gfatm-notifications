...
public class EmailNotificationsJob implements Job {
	...
	public void execute(JobExecutionContext context)
		...
		warehouseInstance.LoadAllUsersEmail();

		dateFrom = new DateTime().minusHours(Constants.EMAIL_SCHEDULE_INTERVAL_IN_HOURS);;
	    dateTo = new DateTime(); 		
	...
	public void setOpenmrsWarehouse(OpenMrsUtil openmrsWarehouse) {
		this.warehouseInstance = openmrsWarehouse;
	}
	...
	public void fastdailyEmailReportExecution(DateTime dateFrom, DateTime dateTo) {
		...
		ArrayList<FactTable> factFast = warehouseInstance.getFactFast(dFrom, dTo);

		for (FactTable factTable : factFast) {
		    //factTable.setEmailAddress(warehouseInstance.getEmailByLocationName(factTable.getLocationId()).getEmailAdress());
	...
	public boolean fastSiteSpecificScreenEmail(FactTable factTable) {
	    ...
	    boolean isSent =	emailController.sendEmailWithHtml(factTable.getEmailAddress(), props.getProperty("mail.subject.title"),
			  message.toString(), props.getProperty("mail.user.username"));
		return isSent;
	}
	...
	public void generateSubtitleSelectionOutput(FactTable factTable){
		...
		String[] columnNames = {"FORMS","Count"};
		 ...
		 for (int i = 0; i < 10; i++) {
			    dataTable[i] =
			        new Object[] {factTable.getTotalVerbalScreen(),factTable.getLocationName()};
			  }
	...
	public StringBuilder getFormatedMessage(FactTable factTable) {
			...
			String nameString = factTable.getLocationDescription()+" ( "+factTable.getLocationName()+")";
			emailString.append("<html><body>"
			 ...
			 emailString.append("<img src='http://chart.apis.google.com/chart?cht=p3&chs=500x250&chdl=first+legend%7Csecond+legend%7Cthird+legend%7Cfourth+legend&chl=first+label%7Csecond+label%7Cthird+label%7Cfourth+label&chco=FF0000|00FFFF|00FF00|CCC,6699CC|CC33FF|CCCC33&chtt=FAST+DAILY+REPORT&chts=000000,24&chd=t:5,10,50,10,30|25,35,45' />");
			 emailString.append("</caption>");
			emailString.append("<caption style='background-color: #32CD32 ; border: 1px solid black;  padding:10px; color: white;font-family:Helvetica, sans-serif;'>");
			 emailString.append(nameString.toUpperCase());
			 emailString.append("</caption>");
			emailString.append("<thead style='background-color: #32CD32'>");
			 emailString.append("<tr>");
				emailString.append("<th style='border: 1px solid black; color: white; padding:10px;font-family:Helvetica, sans-serif;' >FIELDS</th>");
				emailString.append("<th style='border:1px solid black; color:  white; padding:10px;font-family:Helvetica, sans-serif;' >COUNTS</th>");
			emailString.append("</tr>");
			emailString.append("</thead>");
			emailString.append("<tbody style='background-color : #DCDCDC ; border-color: #517994; color:black ;padding:10px;'>");   
				  emailString.append("<tr>");
					    emailString.append("<td style='border:1px solid black; font-family:Helvetica, sans-serif;'>");
			       ...
			       emailString.append("</tr>");
			          ...
			          emailString.append("</tr>");
				       ...
				       emailString.append("<tr>");
					    ...
					    emailString.append("</td>");
					    emailString.append("<td style='border:1px solid black'>");
					    	emailString.append(factTable.getSamplesCollectedVerbalScreenPresumptives());
					    emailString.append("</td>");
			          emailString.append("</tr>");
